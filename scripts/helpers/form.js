...
export function editFormTemplate(selector) {
   ...
   for(let card of arrCardsVisits) {
    ...
    <form id="editForm" class="modal formDoc">    
    ...
    <option class="modal" ${cardObject.doctors === 'DentistVisit' ? 'selected' : ''} value="DentistVisit">Стоматолог</option>
    <option class="modal" ${cardObject.doctors === 'TherapistVisit' ? 'selected' : ''} value="TherapistVisit">Терапевт</option>
    <option class="modal" ${cardObject.doctors === 'CardioVisit' ? 'selected' : ''} value="CardioVisit">Кардіолог</option>
  </select>
    <div class="lableVisit">
    <lable id="nameForm">ПІБ:</lable><br>
    <input type="text" name="fullName" required value="${cardObject.fullName}" ></div>
    <div class="lableVisit">
